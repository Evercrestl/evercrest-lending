                            ...
                            <div className="space-y-6">
                                ...
                                <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                                            ...
                                            <p className="text-2xl font-bold text-emerald-800 mt-2">₱ {amountPaid.toLocaleString()}</p>
                                        </div>
                                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                                            <p className="text-xs text-slate-600 font-semibold uppercase tracking-wider">Remaining</p>
                                            <p className="text-2xl font-bold text-slate-800 mt-2">₱ {currentBalance.toLocaleString()}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm">
                                    ...
                                    <div className="space-y-4">
                                        <DetailRow label="Principal Amount" value={`₱ ${totalLoan.toLocaleString()}`} />
                                        <DetailRow label="Interest Rate" value={`${interestRate}% per annum`} />
                                        <DetailRow label="Total Interest" value={`₱ ${Math.round(totalInterest).toLocaleString()}`} />
                                        <DetailRow 
                                            label="Total Repayment Amount" 
                                            value={`₱ ${totalLoanWithInterest.toLocaleString()}`}
                                            highlight 
                                        />
                                        <DetailRow label="Loan Term" value={`${repaymentMonths} months`} />
                                        <DetailRow label="Monthly Payment" value={`₱ ${monthlyPayment.toLocaleString()}`} />
                                        <DetailRow 
                                            label="Security Deposit" 
                                            value={`₱ ${securityDeposit.toLocaleString()} (${depositPercentage}%)`} 
                                        />
                                        <DetailRow 
                                            label="Loan Status" 
                                            value={
...
function PaymentScheduleRow({ month, date, amount, status, isPast }) {
    ...
    return (
        <div className={`flex items-center justify-between p-4 rounded-xl border transition-all hover:shadow-md ${
            isPast ? 'bg-slate-50 border-slate-200' : 'bg-white border-slate-200'
        }`}>
            <div className="flex items-center gap-4">
